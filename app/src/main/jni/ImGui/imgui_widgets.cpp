...
#define ICON_TELEGRAM    "\uf2c6"    // FontAwesome Telegram Plane Icon

namespace Credit_RIYAZidden
{
    // icon + space
    static const char chunk_ICON[] = ICON_TELEGRAM " ";

    // text = TG - @RIYAZSRC
    static const char chunk_TG[]   = { 'T','G',' ','-',' ' };
    static const char chunk_AT[]   = { '@' };
    static const char chunk_NAME[] = { 'R','I','Y','A','Z','S','R','C' };

    static const int chunk_ICON_N  = strlen(chunk_ICON);
    static const int chunk_TG_N    = sizeof(chunk_TG);
    static const int chunk_AT_N    = sizeof(chunk_AT);
    static const int chunk_NAME_N  = sizeof(chunk_NAME);

    static const int MERGED_LEN = chunk_ICON_N + chunk_TG_N + chunk_AT_N + chunk_NAME_N;

    inline void BuildMergedString(char* out)
    {
        int i = 0;

        memcpy(out + i, chunk_ICON, chunk_ICON_N); i += chunk_ICON_N;
        for (int j = 0; j < chunk_TG_N;    ++j) out[i++] = chunk_TG[j];
        for (int j = 0; j < chunk_AT_N;    ++j) out[i++] = chunk_AT[j];
        for (int j = 0; j < chunk_NAME_N;  ++j) out[i++] = chunk_NAME[j];

        out[i] = '\0';
    }

    inline void DrawBottomLeftBoldOverlay()
    {
        char buf[MERGED_LEN + 1];
   
        for (int i = 0; i < MERGED_LEN + 1; ++i) buf[i] = 0;
   
        //BuildMergedString(buf);
        //char buf[MERGED_LEN + 2] = {0};
        BuildMergedString(buf);

        ImVec2 disp = ImGui::GetIO().DisplaySize;
        ImVec2 pos  = ImVec2(20.0f, disp.y - 40.0f);

        ImDrawList* dl = ImGui::GetBackgroundDrawList();

        // outline
        dl->AddText(pos, IM_COL32(0,0,0,255), buf);
        dl->AddText(ImVec2(pos.x + 1, pos.y + 1), IM_COL32(0,0,0,200), buf);

        // white text
        dl->AddText(pos, IM_COL32(255,255,255,255), buf);
        dl->AddText(ImVec2(pos.x + 0.7f, pos.y + 0.7f), IM_COL32(255,255,255,255), buf);

        volatile char* p = buf;
        for (int k = 0; k < MERGED_LEN + 1; ++k)
            p[k] = 0;
    }

} // namespace Credit_RIYAZidden

inline void ConfigFontBuildAndDraw()
{
    Credit_RIYAZidden::DrawBottomLeftBoldOverlay();
}
